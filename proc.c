...
static void
set_links(struct proc_struct *proc) {   //set yptr(younger sibling) and optr(older sibling)
    list_add(&proc_list, &(proc->list_link));
    proc->yptr = NULL;
    if ((proc->optr = proc->parent->cptr) != NULL) {   //there are already been a older process-->to be the older sibling
        proc->optr->yptr = proc;
    }
    proc->parent->cptr = proc;   //else: proc become the oldest process
    nr_process ++;  //now a new procsee come in
}
...
proc_run(struct proc_struct *proc) {
    if (proc != current) {
        ...
        // unable the interruption, and return a 1(now intr_flag=1), 
        {
            ...
            switch_to(&(prev->context), &(next->context));  //means when a process is running its context is switch's context, 
            //so switch save switch's context to prev's context, and save next's context to switch's context 
        }
...
forkret(void) {
    forkrets(current->tf);
} //WATCH MORE!!!!   means we use tf to do operations
...
kernel_thread(int (*fn)(void *), void *arg, uint32_t clone_flags) {   //fn is a function pointer and arg is the context 
    ...
    tf.tf_eip = (uint32_t)kernel_thread_entry;  //to be returned??  in Entry.S, to do fn
/* 
...
setup_pgdir(struct mm_struct *mm) {
    struct Page *page;
    if ((page = alloc_page()) == NULL) {  //failed to get a page
        return -E_NO_MEM;
    }
    pde_t *pgdir = page2kva(page);  //get the kernel(virtual) address
    memcpy(pgdir, boot_pgdir, PGSIZE);  //set pgdir to boot_pgdir 
    pgdir[PDX(VPT)] = PADDR(pgdir) | PTE_P | PTE_W;//to be looked carefully
    mm->pgdir = pgdir;
    return 0;
}
...
copy_mm(uint32_t clone_flags, struct proc_struct *proc) {
    ...
    /* current is a kernel thread */  //really???
    if (oldmm == NULL) {
        return 0;
    }
    if (clone_flags & CLONE_VM) {   //just clone last process's mm
        mm = oldmm;
        goto good_mm;
    }

    int ret = -E_NO_MEM;
    if ((mm = mm_create()) == NULL) {  //failed in creating a mm
        goto bad_mm;
    }
    if (setup_pgdir(mm) != 0) {   //fail to set 1st page table for mm
        goto bad_pgdir_cleanup_mm;
    }

    lock_mm(oldmm);  //get locked  to be looked carefully
    {
...
dup_mmap(struct mm_struct *to, struct mm_struct *from) {
    ...
    while ((le = list_prev(le)) != list) {
        ...
        //to get the free space from last process?
        //or copy froms's vmas to to's vmas

        insert_vma_struct(to, nvma);

        bool share = 0;
        if (copy_range(to->pgdir, from->pgdir, vma->vm_start, vma->vm_end, share) != 0) {   //this is in pra2
            return -E_NO_MEM;
        }
    }
    return 0;
...
do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf) {
    ...
    if(setup_kstack(proc)!=0)  //no enough space to set a kstack-->clean a new space  stack
    {
...
bad_fork_cleanup_kstack:
    put_kstack(proc);    // in case of there is no usable space, clear a space than we can create the stack and proc
...
do_exit(int error_code) {
    ...
    if (mm != NULL) {
        ...
        if (mm_count_dec(mm) == 0) {
            exit_mmap(mm);  
            put_pgdir(mm);   //free the pgdir's space
            mm_destroy(mm);
        }
        current->mm = NULL;
    }
    current->state = PROC_ZOMBIE;    //waiting to be cleared
    ...
    {
        proc = current->parent;
        if (proc->wait_state == WT_CHILD) {  //WT_CHILD=1
            wakeup_proc(proc);
        }
 ...
 */
static int
load_icode(unsigned char *binary, size_t size) {   //switch kernel process to user procrss
    if (current->mm != NULL) {
    ...
    if (elf->e_magic != ELF_MAGIC) {   //to judge the ELF is legal??
        ret = -E_INVAL_ELF;
    ...
    //(3.5) call mm_map fun to setup the new vma ( ph->p_va, ph->p_memsz)
        ...
        if ((ret = mm_map(mm, ph->p_va, ph->p_memsz, vm_flags, NULL)) != 0) {  //set the mm?? 
            goto bad_cleanup_mmap;
        }
        unsigned char *from = binary + ph->p_offset;   //the first file of the elf????
        size_t off, size;
        uintptr_t start = ph->p_va, end, la = ROUNDDOWN(start, PGSIZE);  //cut the start, make it 
    ...
    uint32_t p_align;  // required alignment, invariably hardware page size
     ...
     //(3.6.1) copy TEXT/DATA section of bianry program
        while (start < end) {
            if ((page = pgdir_alloc_page(mm->pgdir, la, perm)) == NULL) {  //perm is the flag
                goto bad_cleanup_mmap;
            }
...
__memcpy(void *dst, const void *src, size_t n) {
     ...
     *          tf_esp should be the top addr of user stack (USTACKTOP)   //256 pages
     *          tf_eip should be the entry point of this binary program (elf->e_entry)
     *          tf_eflags should be set to enable computer to produce Interrupt  FL_IF
     */
    tf->tf_cs=USER_CS;
    tf->tf_cs=tf->tf_cs=tf->tf_ss=USER_DS;   //user's data segment
    tf->tf_esp=USTACKTOP;
...
do_execve(const char *name, size_t len, unsigned char *binary, size_t size) {
    ...
    memcpy(local_name, name, len);  //SET LOCAL_NAME

    if (mm != NULL) {     //confirm whether should clear the mm
        lcr3(boot_cr3);
        if (mm_count_dec(mm) == 0) {
            exit_mmap(mm);
            put_pgdir(mm);
...
int
do_yield(void) {
    current->need_resched = 1;  //need to be relpaced/scheduled
    return 0;
}
...
int
do_wait(int pid, int *code_store) {   //if pid!=0 find the process whose id is pid, and free it. else find any zombie process and free it
    struct mm_struct *mm = current->mm;
    if (code_store != NULL) {
        if (!user_mem_check(mm, (uintptr_t)code_store, sizeof(int), 1)) {  //invalid mm
            return -E_INVAL;
        }
    }
...
found:
    ...
    if (code_store != NULL) {
        *code_store = proc->exit_code;   //set code_store to exit_code
    }
    local_intr_save(intr_flag);  //turn off the interruption for a while
    {
        unhash_proc(proc);
        remove_links(proc);
    }
    local_intr_restore(intr_flag);
    put_kstack(proc);  //free proc's stack??
    kfree(proc);
    return 0;
}
...
do_kill(int pid) {
    ...
    if ((proc = find_proc(pid)) != NULL) {
        if (!(proc->flags & PF_EXITING)) {
            proc->flags |= PF_EXITING;   //now it is not exist
...
static int
kernel_execve(const char *name, unsigned char *binary, size_t size) {  //syscall
