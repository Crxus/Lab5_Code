...
alloc_pages(size_t n) {
    ...
    while (1)
    {
         local_intr_save(intr_flag);   //lock and unlock of athread??  intr_flag=1
         {
              page = pmm_manager->alloc_pages(n);
         }
         local_intr_restore(intr_flag);  //start the intterrupt???

         if (page != NULL || n > 1 || swap_init_ok == 0) break;    //n>1, failed in getting a new page n>1?????
...
//!!!!!!!!!question here!!!  Get already got the Physical address with lower 12 bits are all 0.
      ...
      //PDE_ADDR-->mmu.h,line 219, To get the value of the index(lower 12 bits are all 0). KADDR-->get the vitual address(lower 12 bits are all 0)
...
copy_range(pde_t *to, pde_t *from, uintptr_t start, uintptr_t end, bool share) {
    assert(start % PGSIZE == 0 && end % PGSIZE == 0);
    assert(USER_ACCESS(start, end));  //user can get access to the space
    // copy content by page unit.
    do {
        //call get_pte to find process A's pte according to the addr start
        pte_t *ptep = get_pte(from, start, 0), *nptep;     //get a pte from is a pd
        if (ptep == NULL) {
            start = ROUNDDOWN(start + PTSIZE, PTSIZE);
            continue ;
        }
        //call get_pte to find process B's pte according to the addr start. If pte is NULL, just alloc a PT
        if (*ptep & PTE_P) {   //the index of this pte is not null
            if ((nptep = get_pte(to, start, 1)) == NULL) {   //can not get a pte, no space and can npt copy
                return -E_NO_MEM;
            }
        uint32_t perm = (*ptep & PTE_USER);
        //get page from ptep
        struct Page *page = pte2page(*ptep);   //get the ph page
        ...
        memcpy((void*)dst_kvaddr,(void*)src_kvaddr,sizeof(struct Page));
        page_insert(to,npage,start,perm);
        assert(ret == 0);
        }
        start += PGSIZE;  //again and again
    } while (start != 0 && start < end);
    return 0;
}
...
page_insert(pde_t *pgdir, struct Page *page, uintptr_t la, uint32_t perm) {
    ...
    if (*ptep & PTE_P) {
        ...
        if (p == page) {
            page_ref_dec(page);  //the connection has already been built
        }
        else {
            page_remove_pte(pgdir, la, ptep);  //relpace the old index to new index
        }
    }
